...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    global ttl, length, packetRcvd, rttMin, rttMax, rttSum, rttCount
    timeLeft = timeout
    ...
    while 1:
       ...
       #ipHeader = recPacket[:20]
        #version, type, length, ipID, flags, ttl, protocol, check_sum, srcIP, destIP = struct.unpack("!BBHHHBBHII", ipHeader)
...
def ping(host, timeout=1):
    global rttMin, rttMax, rttSum, rttCount
    rttMin = float('+inf')
    rttMax = float('-inf')
    rttSum = 0
    rttCount = 0
...
if __name__ == '__main__':
    ping("google.co.il")
    #ping("yahoo.com")
#lab_4_Pinger_skeleton.py
#Displaying lab_4_Pinger_skeleton.py.